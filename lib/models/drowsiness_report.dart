...
class DrowsinessEventsByDay {
  const DrowsinessEventsByDay({required this.date, required this.totalEvents});
...
class DrowsinessEventsByHour {
  const DrowsinessEventsByHour({required this.hour, required this.totalEvents});
...
class WeekdayBehaviorCounts {
  ...
  int get total => drowsiness + yawn + drowsyScoreOn + distraction + other;
...
class DrowsinessEvent {
  ...
  /// Clip recorded around the event. Null until the backend sends one; the
  /// detail panel falls back to [imageUrl] then.
  ...
  factory DrowsinessEvent.fromJson(Map<String, dynamic> json) {
    ...
    return DrowsinessEvent(
      ...
      vehicleId:
          (json['vehicle_identification_number'] ?? json['vehicle_id'] ?? '')
              .toString(),
      ...
      // The API has not settled on a key for the clip yet, so accept the
      // likely ones rather than forcing a redeploy when it lands.
      videoUrl: _optionalString(
        json['video_path'] ?? json['video_url'] ?? json['clip_url'],
      ),
