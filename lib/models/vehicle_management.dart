...
class VehicleRegistryItem {
  ...
  /// Photo of the vehicle. Empty until the registry API returns one; the
  /// detail panel falls back to a type icon.
...
class ManagedVehicle {
  ...
  String get imageUrl => registry.imageUrl;
  ...
  /// How long ago this vehicle last reported, in words. Dashes when the
  /// device has never sent telemetry.
  String get lastSeenLabel {
    final minutes = lastSeenMinutes;
  ...
  /// Last known speed, or null when the device has not reported one.
  double? get speed => status?.speed;
  double? get latitude => status?.latitude;
  double? get longitude => status?.longitude;
