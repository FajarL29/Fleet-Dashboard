...
class MapSection extends StatefulWidget {
  ...
  /// Overrides the basemap tiles. Defaults to the standard OSM raster tiles.
  final String? tileUrlTemplate;
  final List<String> tileSubdomains;

  /// Deepest zoom the tile source actually ships. Past it flutter_map upscales
  /// the last real tile instead of requesting one the server does not have.
  final int? tileMaxNativeZoom;

  /// Attribution line required by the tile provider's terms of use.
  final String? tileAttribution;

  /// Color painted over the basemap, e.g. to tint it toward the app palette.
  final Color? tileOverlayColor;

  /// Set to false to hide the fullscreen / fit-all floating buttons.
  ...
  const MapSection({
    ...
    this.tileMaxNativeZoom,
    this.tileAttribution,
    this.tileOverlayColor,
...
class _MapSectionState extends State<MapSection> with TickerProviderStateMixin {
  ...
  // Matches the marker tween so the followed vehicle stays centred while it slides.
  static const Duration _followDuration = Duration(milliseconds: 500);
  ...
  Vehicle? _selectedVehicleData(String? selectedVehicleId) =>
      _vehicleIn(widget.vehicles, selectedVehicleId);

  static Vehicle? _vehicleIn(List<Vehicle> vehicles, String? id) {
  ...
  /// Glides the camera onto [target], keeping whatever zoom the user chose.
  void _followTo(LatLng target) {
    if (!_isFollowingMode) return;
  ...
  void didUpdateWidget(MapSection oldWidget) {
    ...
    if (_isFollowingMode) {
      final selectedId = _effectiveSelectedVehicleId;
      final previous = _vehicleIn(oldWidget.vehicles, selectedId);
      final current = _vehicleIn(widget.vehicles, selectedId);
      if (previous != null &&
          current != null &&
          previous.position != current.position) {
        _afterFrame(() => _followTo(current.position));
      }
    }

    final shouldRefit = oldWidget.vehicles.length != widget.vehicles.length;
    final shouldFollow =
        !widget.useLocalSelection &&
        oldWidget.selectedVehicleId != widget.selectedVehicleId &&
        widget.selectedVehicleId != null;

    // Deferred to after the frame, not run here.
    //
    // didUpdateWidget executes inside the build phase, and both of these end
    // up notifying an ancestor — onClearSelection raises a bloc event,
    // onFollowModeChanged calls the parent's setState. Doing that mid-build
    // marks a parent dirty while it is already building, which Flutter
    // rejects outright. It stayed hidden until the vehicle list started
    // changing length at runtime, which live GPS made routine.
    if (shouldRefit || shouldFollow) {
      _afterFrame(() {
        if (shouldRefit) _fitAllVehicles();
        if (shouldFollow) _setFollowingMode(true);
      });
    }

    // Local state only, so this one is safe to do right here.
  ...
  /// Runs [action] once the current frame is done, if this is still mounted.
  ...
  void _setFollowingMode(bool enabled) {
    if (!enabled) _followController?.stop();
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
                  ...
                  child: Listener(
                    ...
                    child: FlutterMap(
                      ...
                      children: [
                        TileLayer(
                          urlTemplate:
                              widget.tileUrlTemplate ??
                              'https://tile.openstreetmap.org/{z}/{x}/{y}.png',
                          subdomains: widget.tileSubdomains,
                          maxNativeZoom: widget.tileMaxNativeZoom ?? 19,
                          userAgentPackageName: 'com.fleet.dashboard',
                        ),
                        if (widget.tileOverlayColor != null)
                          IgnorePointer(
                            child: Container(color: widget.tileOverlayColor),
                          ),
        ...
        if (widget.tileAttribution != null)
          Positioned(
            right: 6,
            bottom: 4,
            child: IgnorePointer(
              child: Text(
                widget.tileAttribution!,
                style: TextStyle(
...
class _SmoothVehicleMarkerLayerState extends State<SmoothVehicleMarkerLayer>
  ...
  Widget _buildMarkerWidget(Vehicle vehicle) {
    ...
    // A remembered position, not a live one: the device has gone quiet and
    // this is simply where it was last heard from.
    final isLastKnown =
        vehicle.statusReason?.startsWith(kLastKnownPrefix) ?? false;
...
class _VehicleMarkerWidget extends StatelessWidget {
  ...
  /// Whether this dot is where the vehicle was last seen rather than where it
  /// is. Drawn faded, with a dashed-looking ring and a clock face instead of
  /// the heading arrow — a position nobody can confirm should not look like a
  /// position somebody can.
  ...
  Widget build(BuildContext context) {
    final marker = AnimatedRotation(
      // A stale marker keeps no bearing: the vehicle stopped reporting, so the
      // last heading says nothing about which way it is pointing now.
      turns: isLastKnown ? 0 : heading / 360,
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: color.withValues(alpha: isLastKnown ? 0.45 : 0.9),
          shape: BoxShape.circle,
          border: Border.all(
            color: isSelected
                ? Colors.yellow
                : Colors.white.withValues(alpha: isLastKnown ? 0.6 : 1),
        ...
        child: Icon(
          isLastKnown ? Icons.history_toggle_off_rounded : Icons.navigation,
          color: Colors.white.withValues(alpha: isLastKnown ? 0.75 : 1),
          size: 18,
        ),
      ),
    );
